...
- The actual image processing and uploading occur in a background thread, allowing for a quick response from the API.
- The `gcs_path` can be used to reference or retrieve the image once the upload is complete.
- For URL format, the API downloads the image from the URL provided by DALL-E before uploading to GCS.
- For b64_json format, the API decodes the base64 string into an image file before uploading to GCS.

## Error Handling

If the image generation fails, the API will respond with a 500 status code and an error message:
...
Always check the response status code to ensure successful processing of your request.
