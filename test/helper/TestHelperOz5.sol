    ...
    function setUpEndpoints(uint8 _endpointNum, LibraryType _libraryType) public {
        ...
        for (uint8 i = 0; i < _endpointNum; i++) {
            if (_libraryType == LibraryType.UltraLightNode) {
                ...
                libSetup.sendUln =
                    new SendUln302(payable(this), endpointAddr, TREASURY_GAS_CAP, TREASURY_GAS_FOR_FEE_CAP);
...

                {
                    ...
                    libSetup.executor = new Executor(
                        endpointAddr, address(0x0), messageLibs, address(endpointSetup.priceFeed), address(this), admins
                    );
                    ...
                    libSetup.dvn =
                        new DVN(i + 1, messageLibs, address(endpointSetup.priceFeed), endpointSetup.signers, 1, admins);
                ...
                for (uint8 j = 0; j < _endpointNum; j++) {
                    ...
                    // dvn config
                    configParams.dvnConfigParams[j] =
                ...
                SimpleMessageLibMock messageLib =
                    new SimpleMessageLibMock(payable(this), address(endpointSetup.endpointList[i]));
